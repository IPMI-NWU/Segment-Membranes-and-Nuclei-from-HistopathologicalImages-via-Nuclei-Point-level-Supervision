...
color_dict={'0': (0, 51, 255), '1': (255, 0, 0), '2': (255, 0, 255), '3': (64, 81, 181),'4': (181,64, 81)}
...
def compare_with_annotations(test_data_dir, anno_dir,test_name, net, cell_count_dict=None, is_display=False):

    result_save_dir = opt.test['save_dir']
    ...
    for image_path in image_list:
        ...
        json_fullname = image_fullname.replace('png', 'json')

        annotation_path = image_path.replace(test_data_dir, anno_dir).replace('.png', '.json')
        print("evaluating image {}".format(image_path))
        annotation_count_dict, annotation_center_coords, annotation_labels = count_annotation_summary(label_dict,
                                                                                                      annotation_path,
                                                                                                      annotation_count_dict)
        cell_count_dict, test_center_coords, test_labels,cell_masks,membrane_masks = count_test_summary(image_path, net, test_count_dict)
        result_dict['cell_count_dict'] = cell_count_dict
        result_dict['test_center_coords'] = test_center_coords.tolist()
        result_dict['test_labels'] = test_labels.tolist()
        ...
        if is_display:
            ...
            annotation_img = draw_center(image, annotation_center_coords, annotation_labels)
            final_masks=np.zeros_like(image).astype(np.float)
            membrane_masks = np.where(membrane_masks[:, :, 0] < 0., 1, 0).astype(np.uint8)
            cell_masks = np.where(cell_masks[:, :, 0] > 0.5, 1, 0).astype(np.uint8)
            membrane_masks=membrane_masks-cell_masks
            final_masks[:, :, 0] = membrane_masks * 255
            final_masks[:, :, 1] = cell_masks * 255
            blank = np.ones((h, 20, c)) * 255
            result_img = np.hstack((image, blank,annotation_img,blank,test_res_img,blank,final_masks))
            imsave(os.path.join(result_save_dir, os.path.splitext(os.path.basename(image_path))[0] + '.png'),
                   result_img)
...
def count_test_summary(image_path, net,cell_count_dict):
    image = imread(image_path)
...
def run_test(test_dir,anno_dir, weight_path):
    ...
    compare_with_annotations(test_data_dir=test_dir, anno_dir=anno_dir,test_name=test_name,net=net, is_display=True)
...
if __name__ == "__main__":
    ...
    test_dir = "../data_memb/test/images"
    anno_dir="../data_memb/test/labels"

    run_test(test_dir, anno_dir,
             weight_path=opt.test['model_path'])
    # run_test(test_dir, 1,
